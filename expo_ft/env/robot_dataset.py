"""Generalized offline HDF5 dataset loader (DROID, sim, Piper).

Episodes live in numeric directories under `datapath`, each containing a
`traj.hdf5` with `saved_observation/*` and `action/*` groups. Terminal
reward/done are synthesized per episode (reward 1 on the last step).
"""
...
def process_robot_hdf5_dataset(
    datapath,
    task_config,
...
# Backward-compatible alias (upstream name).
process_droid_dataset = process_robot_hdf5_dataset
